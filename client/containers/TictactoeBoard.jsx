...
class TictactoeBoard extends React.Component {
  ...
  winnerCheck() {
    ...
    for (let i=0; i < 9; i+=3) {
      if (this.state.squares[i+0]===this.state.squares[i+1] && this.state.squares[i+1]===this.state.squares[i+2]) {
        this.setState((prevState) => {return {winner: this.state.squares[i+0]}})
      }
    }
    // Check columns for winners
    for (let i=0; i < 3; i++) {
      if (this.state.squares[0+i]===this.state.squares[3+i] && this.state.squares[3+i]===this.state.squares[6+i]) {
        this.setState((prevState) => {return {winner: this.state.squares[0+i]}})
      }
    }
    // Check diagonals for winners
    if (this.state.squares[0]===this.state.squares[4] && this.state.squares[4]===this.state.squares[8]) {
      this.setState((prevState) => {return {winner: this.state.squares[4]}})
    }
    if (this.state.squares[2]===this.state.squares[4] && this.state.squares[4]===this.state.squares[6]) {
      this.setState((prevState) => {return {winner: this.state.squares[4]}})
    }
  }
  ...
  render () {
    ...
    for (let i = 1; i < 10; i += 3 ){
      ...
      var squares = [];
      for (let k = 0; k < 3; k++ ){
        squares.push(<td key={ k+ i}><TictactoeSquare position={ k + i }  crossesTurn = { this.state.crossesTurn} changeTurn={this.changeTurn} recordMove={this.recordMove}/></td>);
        //  table.push(square)
      }
